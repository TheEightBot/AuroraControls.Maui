...
public class SKSvg
{
    ...
    private static readonly XNamespace _xlinkNamespace = "http://www.w3.org/1999/xlink";
    ...
    private readonly Dictionary<string, XElement> _defs = new Dictionary<string, XElement>();
    private readonly Dictionary<string, ISKSvgFill> _fillDefs = new Dictionary<string, ISKSvgFill>();
    private readonly Dictionary<XElement, string> _elementFills = new Dictionary<XElement, string>();
    ...
    {
        using (var reader = XmlReader.Create(stream, _xmlReaderSettings, CreateSvgXmlContext()))
        {
            Load(reader);
        }
    }
    ...
    private static XmlParserContext CreateSvgXmlContext()
    {
        ...
        manager.AddNamespace(string.Empty, _svgNamespace.NamespaceName);
        manager.AddNamespace("xlink", _xlinkNamespace.NamespaceName);
    ...
    {
        _svg = xdoc.Root;
        var ns = _svg.Name.Namespace;

        // find the defs (gradients) - and follow all hrefs
        foreach (var d in _svg.Descendants())
        {
            var id = d.Attribute("id")?.Value?.Trim();
            if (!string.IsNullOrEmpty(id))
            {
                _defs[id] = ReadDefinition(d);
            }
        }

        Version = _svg.Attribute("version")?.Value;
        Title = _svg.Element(ns + "title")?.Value;
        Description = _svg.Element(ns + "desc")?.Value ?? _svg.Element(ns + "description")?.Value;

        // TODO: parse the "preserveAspectRatio" values properly
        _preserveAspectRatio = _svg.Attribute("preserveAspectRatio")?.Value;

        // get the _svg dimensions
        var viewBoxA = _svg.Attribute("viewBox") ?? _svg.Attribute("viewPort");
        if (viewBoxA != null)
        {
        ...
        if (CanvasSize.IsEmpty)
        {
            ...
            var widthA = _svg.Attribute("width");
            var heightA = _svg.Attribute("height");
    ...
    public SKPicture Draw()
    {
        ...
        using (var canvas = recorder.BeginRecording(SKRect.Create(CanvasSize)))
        {
            ...
            // scale the _svg dimensions to fit inside the user dimensions
            if (!ViewBox.IsEmpty && (ViewBox.Width != CanvasSize.Width || ViewBox.Height != CanvasSize.Height))
            {
                if (_preserveAspectRatio == "none")
                {
            ...
            var style = ReadPaints(_svg, ref stroke, ref fill, true);

            // read elements
            LoadElements(_svg.Elements(), canvas, stroke, fill);
        ...
        switch (elementName)
        {
            ...
            case "line":
                if (stroke != null || fill != null)
                {
                    ...
                    if (_elementFills.TryGetValue(e, out var fillId) && _fillDefs.TryGetValue(fillId, out var addFill))
                    {
    ...
    private static bool HasSvgNamespace(XName name)
    {
        return
            ...
            name.Namespace == _svgNamespace ||
            name.Namespace == _xlinkNamespace;
    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (urlM.Success)
                    {
                        ...
                        if (_defs.TryGetValue(id, out var defE))
                        {
                            switch (defE.Name.LocalName.ToLower())
                            {
                                case "lineargradient":
                                    _fillDefs[id] = ReadLinearGradient(defE);
                                    fillId = id;
                                    read = true;
                                    break;
                                case "radialgradient":
                                    _fillDefs[id] = ReadRadialGradient(defE);
                                    fillId = id;
                                    read = true;
                                    break;
                            }
    ...
    private SKPath ReadClipPath(string raw)
    {
        ...
        if (urlM.Success)
        {
            ...
            if (_defs.TryGetValue(id, out XElement defE))
            {
    ...
    private XElement ReadHref(XElement e)
    {
        ...
        if (string.IsNullOrEmpty(href) || !_defs.TryGetValue(href, out XElement child))
        {
            child = null;
        }
    ...
    {
        return (e.Attribute("href") ?? e.Attribute(_xlinkNamespace + "href"))?.Value;
    }
